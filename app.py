...
# ==========================================
# 2. 웹페이지 기본 설정 및 [보고용 프리미엄 CSS 주입]
# ==========================================
...
# 4MB 경량 AI 모델 캐싱 함수 (서버 메모리 보호)
@st.cache_resource
def get_light_session():
...
with tab1:
  ...
  uploaded_file = st.file_uploader(
      "스낵 사진을 업로드하거나 촬영하세요", type=["jpg", "jpeg", "png"]
  )
  ...
  if uploaded_file is not None:
    ...
    with st.spinner("AI가 시즈닝 분포와 색차(ΔE)를 정밀 분석 중입니다..."):
      ...
      # AI 분석 엔진 (경량 u2netp 모델 적용)
      session = get_light_session()
      ...
      mean_delta_e = (
          float(np.mean(valid_pixels)) if len(valid_pixels) > 0 else 0
      )
      ...
      ratio = np.clip(
          (a_channel - min_redness) / (max_redness - min_redness), 0, 1
      )
      ...
      heatmap_masked = cv2.bitwise_and(
          blended_heatmap, blended_heatmap, mask=mask
      )
    ...
    with img_col2:
      ...
      st.image(
          cv2.cvtColor(black_bg_img, cv2.COLOR_BGR2RGB),
          use_container_width=True,
      )
    ...
    with col_tile:
      ...
      st.markdown(
          f"<div style='text-align: center; margin-top: 10px;'>L: {avg_l:.0f}"
          f" &nbsp;&nbsp; a: {avg_a:.0f} &nbsp;&nbsp; b: {avg_b:.0f}</div>",
          unsafe_allow_html=True,
      )
    ...
    with col_dist:
      ...
      ax.hist(
          valid_pixels, bins=60, color="#112A46", edgecolor="white", linewidth=0.5
      )
    ...
    with col_map:
      ...
      st.image(
          cv2.cvtColor(heatmap_masked, cv2.COLOR_BGR2RGB),
          use_container_width=True,
      )
